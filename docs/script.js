...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function toggleMenu() {
    ...
    burgerBtn.setAttribute("aria-expanded", isShown);
    // При открытом меню блокируем скролл страницы (по желанию)
    document.body.style.overflow = isShown ? "hidden" : "";
  }

  // Клик по бургеру
  burgerBtn.addEventListener("click", (e) => {
    e.stopPropagation(); // чтобы клик не "проваливался" дальше
    toggleMenu();
  });

  // Клик по ссылкам меню — закрываем меню
  ...
  // Закрыть меню по клику вне меню и вне кнопки
  document.addEventListener("click", (e) => {
    if (navbarMenu.classList.contains("show")) {
      if (!navbarMenu.contains(e.target) && e.target !== burgerBtn) {
        navbarMenu.classList.remove("show");
        burgerBtn.setAttribute("aria-expanded", "false");
        document.body.style.overflow = "";
      }
